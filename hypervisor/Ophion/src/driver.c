    ...
    _In_ PUNICODE_STRING registry_path)
{
    ...
    //
    // Detect manually mapped mode: kdmapper passes (base_addr, size)
    // instead of real DriverObject/RegistryPath. A valid DRIVER_OBJECT
    // has Type == IO_TYPE_DRIVER (4). If not, we're manually mapped.
    //
    if (driver_obj == NULL || (ULONG_PTR)driver_obj < 0xFFFF800000000000ULL ||
        *(USHORT *)driver_obj != 4 /* IO_TYPE_DRIVER */)
    {
        mapped_mode = TRUE;
        DbgPrintEx(0, 0, "[hv] Manually mapped mode detected\n");
    }
